...
/*
dynamic programing
1.  마지막 계단 직전 계단을 밟은 경우
	-> 마지막-2 번째 계단은 밟을 수 없음
	dp[i - 3] + score[i] + score[i - 1]
2.  마지막 계단 직전 계단을 밟지 않은 경우
	dp[i - 2] + score[i]
*/
#include <iostream>
#include <algorithm>
...
int main()
{
	...
	dp[0] = score[0]; // 1층
	dp[1] = max(score[0] + score[1], score[1]); // 2층
	dp[2] = max(score[0] + score[2], score[1] + score[2]); // 3층
