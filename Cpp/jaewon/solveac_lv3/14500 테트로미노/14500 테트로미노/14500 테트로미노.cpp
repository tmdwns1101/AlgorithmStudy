...
int dfs(int y, int x, int cnt) {
	...
	for (int i = 0; i < 4; i++) {
		...
		if (nx < m && ny < n && nx >= 0 && ny >= 0 && !visited[ny][nx]) {
			visited[ny][nx] = true;
			sum = max(sum, block[y][x] + dfs(ny, nx, cnt + 1));
			visited[ny][nx] = false;
		}
	}
	return sum;
...
int Tblock(int y, int x) {

	int result = 0;
	
	// ㅗ
	if (y >= 1 && x >= 1 && x < m - 1)
		result = max(result, block[y][x - 1] + block[y][x] + block[y - 1][x] + block[y][x + 1]);
	
	// ㅏ
	if (y >= 1 && y < n - 1 && x < m - 1)
		result = max(result, block[y - 1][x] + block[y][x] + block[y + 1][x] + block[y][x + 1]);
